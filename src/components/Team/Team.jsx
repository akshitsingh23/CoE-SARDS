            ...
            <div className='flex flex-col items-center '>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                {teamMembers.administration.map((member, index) => {
                  const isLastElement = index === teamMembers.administration.length - 1;
                  const isOdd = teamMembers.administration.length % 2 !== 0;
                  return (
                    <div
                      key={index}
                      className={`outTeam__card hover:scale-105 hover:text-orange-200 transition duration-200 ease-in-out bg-gradient-to-br from-gray-900 to-gray-700 shadow-lg rounded-lg p-5 ${isLastElement && isOdd ? 'md:col-span-2 md:justify-self-center lg:col-span-2 lg:justify-self-center' : ''}`}
                    >
                      <div className="flex flex-col justify-between h-full">
                        <div className="grid grid-cols-1 justify-items-center">
                          <img
                            src={member.imgSrc}
                            alt={member.name}
                            className="outTeam__cardImg fa-border border-yellow-500 border-4"
                          />
                          <div className='p-2 font-montserrat font-bold rounded text-center'>{member.name}</div>
                          <div className='font-montserrat rounded text-center'>{member.designation}</div>
                        </div>
            ...
            <div className='flex flex-col items-center '>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                {teamMembers.professorsOfPractice.map((member, index) => {
                  const isLastElement = index === teamMembers.professorsOfPractice.length - 1;
                  const isOdd = teamMembers.professorsOfPractice.length % 2 !== 0;
                  return (
                    <div
                      key={index}
                      className={`outTeam__card hover:scale-105 hover:text-blue-200 transition duration-200 ease-in-out bg-gradient-to-br from-gray-800 to-gray-700 shadow-lg rounded-lg p-5 ${isLastElement && isOdd ? 'md:col-span-2 md:justify-self-center lg:col-span-2 lg:justify-self-center' : ''}`}
                    >
                      <div className="flex flex-col justify-between h-full">
                        <div className="grid grid-cols-1 justify-items-center">
                          <img
